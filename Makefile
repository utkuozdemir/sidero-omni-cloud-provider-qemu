...
.PHONY: omni-cloud-provider-qemu-linux-amd64
omni-cloud-provider-qemu-linux-amd64: $(ARTIFACTS)/omni-cloud-provider-qemu-linux-amd64  ## Builds executable for omni-cloud-provider-qemu-linux-amd64.

.PHONY: $(ARTIFACTS)/omni-cloud-provider-qemu-linux-arm64
$(ARTIFACTS)/omni-cloud-provider-qemu-linux-arm64:
	@$(MAKE) local-omni-cloud-provider-qemu-linux-arm64 DEST=$(ARTIFACTS)

.PHONY: omni-cloud-provider-qemu-linux-arm64
omni-cloud-provider-qemu-linux-arm64: $(ARTIFACTS)/omni-cloud-provider-qemu-linux-arm64  ## Builds executable for omni-cloud-provider-qemu-linux-arm64.
...
image-omni-cloud-provider-qemu:  ## Builds image for omni-cloud-provider-qemu.
	@$(MAKE) target-$@ TARGET_ARGS="--tag=$(REGISTRY)/$(USERNAME)/omni-cloud-provider-qemu:$(IMAGE_TAG)"
